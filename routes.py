...
@login_required
def remove_from_collection(user_id, card_id):
    conn = get_db_connection()
    if conn:
        cursor = conn.cursor()
...
def add_card_to_collection(user_id, card_id):
    ...
    if conn:
        cursor = conn.cursor()
        cursor.execute("INSERT INTO user_collections (user_id, card_id, count) VALUES (%s, %s, 1) ON DUPLICATE KEY UPDATE count = count + 1",
                       (user_id, card_id))
        conn.commit()
        cursor.close()
        conn.close()

def remove_card_from_collection(user_id, card_id):
    conn = get_db_connection()
    if conn:
        cursor = conn.cursor()
        cursor.execute("DELETE FROM user_collections WHERE user_id = %s AND card_id = %s", (user_id, card_id))
